/*
	Copyright (c) 2016-2017 Keith Tschohl
...
func makeFlarmPFLAAString(ti TrafficInfo) (msg string, valid bool) {
	...
	*/
	...
	// Enable lowest alarm level (1) for traffic within 0.5 nautical miles and 1000' vertically. Levels 2 and 3 currently unused.
	...
	// TL;DR: There's no one setting that will please everyone. Change this if you don't like it.

	if (dist < 926) && (relativeVertical < 304) && (relativeVertical > -304) { // 926 m = 0.5 NM; 304 = 1000'
		alarmLevel = 1
	}
	...
	//msg = fmt.Sprintf("PFLAA,%d,%d,%d,%d,%d,%X,%d,,%d,,%d", alarmLevel, relativeNorth, relativeEast, relativeVertical, idType, ti.Icao_addr, ti.Track, groundSpeed, acType) // basic message type
	msg = fmt.Sprintf("PFLAA,%d,%d,%d,%d,%d,%X!%s,%d,,%d,%0.1f,%d", alarmLevel, relativeNorth, relativeEast, relativeVertical, idType, ti.Icao_addr, ti.Tail, ti.Track, groundSpeed, climbRate, acType) // extended message type; might not be compatible with all systems.
	// bearingless_msg = fmt.Sprintf("PFLAA,%d,%d,,%d,%d,%X!%s,,,,,%d", alarmLevel, int16(dist), relativeVertical, idType, ti.Icao_addr, ti.Tail, acType) // this will generate a 'bearingless' traffic message for aircraft without known position -- future use?
	for i := range msg {
